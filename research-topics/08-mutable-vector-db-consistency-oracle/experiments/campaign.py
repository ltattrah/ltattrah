...
def main(argv: list[str] | None = None) -> int:
    ...
    with Run(out_dir=args.out, config=config, seed=args.seeds[0], extra={"topic": 8, "tool": "campaign"}) as run:
            ...
            for mi in args.intensities:
                for seed in args.seeds:
                    ...
                    for f in res.findings:
                        if f.failure != FailureClass.APPROXIMATION:
                            findings_f.write(json.dumps({"engine": engine, "seed": seed, "mutation_intensity": mi, **f.as_row()}) + "\n")
        ...
        summary_f.close(); findings_f.close()
